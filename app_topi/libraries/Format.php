...
class Format {
     ...
     * HTML output format
     */
    const HTML5_FORMAT = 'html5';
    const CHART_FORMAT = 'chart';
     ...
     * @return mixed
     */
    public function to_chart($data = NULL)
    {
        // If no data is passed as a parameter, then use the data passed
        // via the constructor
        if ($data === NULL && func_num_args() === 0)
        ...
        $template = array(
                'table_open'            => '<table class="highchart" data-graph-container-before="1" data-graph-type="column" style="display:none">',
        ...
        $this->_CI->table->set_template($template);
        $headings = @array_map(array('Format','dash_to_space'), $headings);
        ...
        foreach ($data as $row)
        {
            ...
            // Keep the "evil" @ here
            $row = @array_map('strval', $row);
            // $row = @array_map(array('Format','_nomor'), $row);
     ...
     * @return mixed
     */
    public function to_html5($data = NULL)
    {
        // If no data is passed as a parameter, then use the data passed
        // via the constructor
        if ($data === NULL && func_num_args() === 0)
    ...
    protected function _nomor($val='')
    {
        return is_numeric($val) ? number_format($val,0,',','.') : $val;
    }

    protected function dash_to_space($val='')
    {
